...
public class PenguinArea : MonoBehaviour
{
    ...
    public Fish poisonFishPrefab;

    private List<GameObject> fishList;
    private List<GameObject> poisonFishList;
    ...
    public void RemoveSpecificPoisonFish(GameObject poisonFishObject)
    {
        poisonFishList.Remove(poisonFishObject);
        Destroy(poisonFishObject);
    }
    ...
    private void RemovePoisonFish()
    {
        if(poisonFishList != null)
        {
            for(int i = 0; i < poisonFishList.Count; i++)
            {
                if(poisonFishList[i] != null)
                {
                    Destroy(poisonFishList[i]);
                }
            }
        }

        poisonFishList = new List<GameObject>();
    }
    ...
    private void SpawnPoisonFish(float fishSpeed)
    {
        GameObject poisonFishObject = Instantiate<GameObject>(poisonFishPrefab.gameObject);
        poisonFishObject.transform.position = ChooseRandomPosition(transform.position, 100f, 260f, 2f, 13f) + Vector3.up * 5f;
        poisonFishObject.transform.rotation = Quaternion.Euler(0f, Random.Range(0f, 360f), 0f);

        poisonFishObject.transform.SetParent(transform);

        poisonFishList.Add(poisonFishObject);
        poisonFishObject.GetComponent<Fish>().fishSpeed = fishSpeed;
    }
